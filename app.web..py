...
# Cálculo de calorías menos necesarias (1kg de grasa aprox. 7700 kcal)
diferencia_peso = peso_actual - peso_ideal
if diferencia_peso > 0:
    calorias_totales_deficit = diferencia_peso * 7700
    deficit_diario = calorias_totales_deficit / (semanas_meta * 7)
    mantenimiento_estimado = peso_actual * 30
    meta_calorias_diarias = mantenimiento_estimado - deficit_diario
    
    st.sidebar.markdown(f"💡 **Cálculo:** Debes comer unos **{deficit_diario:.0f} kcal menos** al día.")
    st.sidebar.markdown(f"🎯 **Meta diaria sugerida:** ~**{meta_calorias_diarias:.0f} kcal**")
else:
    meta_calorias_diarias = peso_actual * 30
    st.sidebar.info("Tu peso ideal es igual o mayor a tu actual. Mantén un consumo equilibrado.")
...
if texto_ingresado:
    ...
    if alimentos_encontrados:
        ...
        st.sidebar.warning("No reconocí ningún alimento registrado en lo que escribiste.")
